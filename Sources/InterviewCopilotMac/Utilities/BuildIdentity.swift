...
struct BuildIdentity: Hashable {
    static let expectedBundleIdentifier = "com.langcheng.InterviewCopilotMac"
    static let expectedDistBundleSuffix = "/dist/InterviewCopilotMac.app"
    ...
    var staleWarning: String? {
        ...
        if latestSourceModifiedAt.timeIntervalSince(executableModifiedAt) > 5 {
            return "This app bundle may be stale. Rebuild and relaunch from dist/InterviewCopilotMac.app."
        }
        return nil
    }
    ...
    static func current(bundle: Bundle = .main, fileManager: FileManager = .default) -> BuildIdentity {
        ...
        let buildTimestampUTC = info["ICBuildTimestampUTC"] as? String ?? "Unknown"
        let gitCommitHash = info["ICGitCommitHash"] as? String ?? "Unknown"
        let gitBranch = info["ICGitBranch"] as? String ?? "Unknown"
        let sourceRoot = info["ICSourceRoot"] as? String ?? Self.inferSourceRoot(bundlePath: bundlePath, fileManager: fileManager)
        let expectedBundlePath = info["ICExpectedBundlePath"] as? String
    ...
    private static func expectedBundlePath(sourceRoot: String, bundlePath: String) -> String {
        if sourceRoot != "Unknown" {
            return URL(fileURLWithPath: sourceRoot)
                .appendingPathComponent("dist/InterviewCopilotMac.app")
                .path
        }
        return bundlePath
    }
    ...
    private static func inferSourceRoot(bundlePath: String, fileManager: FileManager) -> String {
        ...
        for _ in 0..<8 {
            ...
            let sources = current.appendingPathComponent("Sources/InterviewCopilotMac").path
