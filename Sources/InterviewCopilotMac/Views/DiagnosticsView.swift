    ...
    private var currentGenerationTab: some View {
        VStack(alignment: .leading, spacing: 14) {
            card("Current Generation", icon: "sparkles") {
                ...
                diagnosticRow("activeCandidateProfileId", appState.activeCandidateProfileID ?? "None")
                diagnosticRow("activeCandidateProfileVersion", appState.candidateProfiles.first(where: { $0.id == appState.activeCandidateProfileID }).map { "\($0.version)" } ?? "None")
                diagnosticRow("activeOpportunityContextId", appState.activeOpportunityContextID ?? "None")
                diagnosticRow("activeOpportunityContextVersion", appState.opportunityContexts.first(where: { $0.id == appState.activeOpportunityContextID }).map { "\($0.version)" } ?? "None")
                diagnosticRow("contextSnapshotId", appState.activeContextSnapshot?.id ?? appState.currentSession?.contextSnapshotID ?? "None")
                diagnosticRow("domainProfileId", appState.activeInterviewDomainID.rawValue)
                diagnosticRow("candidateEvidenceCount", "\(appState.contextReadiness.candidateFactCount)")
                diagnosticRow("opportunityEvidenceCount", "\(appState.contextReadiness.opportunityRequirementCount)")
                diagnosticRow("automaticContextReadiness", appState.automaticContextReadiness.rawValue)
                diagnosticRow("contextExtractionProvider", appState.automaticContextBuildResult?.metrics.usedLocalQwen == true ? "local_qwen" : "verified_local_extractor")
                diagnosticRow("profileExtractionMs", appState.automaticContextBuildResult.map { "\($0.metrics.profileExtractionMS)" } ?? "None")
                diagnosticRow("opportunityExtractionMs", appState.automaticContextBuildResult.map { "\($0.metrics.opportunityExtractionMS)" } ?? "None")
                diagnosticRow("domainInferenceMs", appState.automaticContextBuildResult.map { "\($0.metrics.domainInferenceMS)" } ?? "None")
                diagnosticRow("contextUncertainFactCount", appState.automaticContextBuildResult.map { "\($0.evidenceSummary.uncertainFactCount)" } ?? "None")
                diagnosticRow("contextConflictCount", appState.automaticContextBuildResult.map { "\($0.evidenceSummary.conflictCount)" } ?? "None")
                diagnosticRow("contextExtractionCacheHits", appState.automaticContextBuildResult.map { "\($0.metrics.extractionCacheHitCount)" } ?? "None")
                diagnosticRow("retrievedCandidateEvidenceIDs", appState.currentSuggestion?.candidateEvidenceIDs.joined(separator: ", ") ?? "None")
                diagnosticRow("retrievedOpportunityEvidenceIDs", appState.currentSuggestion?.opportunityEvidenceIDs.joined(separator: ", ") ?? "None")
                diagnosticRow("groundingDecision", appState.currentSuggestion?.groundingDecision ?? "None")
                diagnosticRow("unsupportedClaimCount", appState.currentSuggestion?.unsupportedClaimCount.map(String.init) ?? "None")
                diagnosticRow("contextIsolationStatus", appState.currentSuggestion?.contextIsolationStatus ?? "None")
            ...
            card("Current QA Binding", icon: "link") {
                ...
                diagnosticRow("selectedSessionMode", appState.interviewSessionMode.rawValue)
                diagnosticRow("resolvedSessionMode", appState.resolvedInterviewSessionPhase.rawValue)
                diagnosticRow("detectedSpeakerRole", appState.detectedDialogueSpeakerRole.rawValue)
                diagnosticRow("detectedTurnType", appState.detectedDialogueTurnType.rawValue)
                diagnosticRow("sourceChannel", appState.lastDialogueSourceChannel.isEmpty ? "None" : appState.lastDialogueSourceChannel)
                diagnosticRow("triggerDecision", appState.lastTriggerDecision.isEmpty ? "None" : appState.lastTriggerDecision)
                diagnosticRow("triggerReason", appState.lastTriggerReason.isEmpty ? "None" : appState.lastTriggerReason)
                diagnosticRow("suppressionReason", appState.lastSuppressionReason.isEmpty ? "None" : appState.lastSuppressionReason)
                diagnosticRow("lastModeTransition", appState.lastModeTransition.isEmpty ? "None" : appState.lastModeTransition)
                diagnosticRow("modeTransitionReason", appState.lastModeTransitionReason.isEmpty ? "None" : appState.lastModeTransitionReason)
                diagnosticRow("candidateQuestionsToPanelCount", "\(appState.candidateQuestionsToPanelCount)")
                diagnosticRow("interviewContextMode", appState.interviewContextMode.rawValue)
                diagnosticRow("answerPanelQuestions", appState.answerPanelQuestionsEnabled ? "true" : "false")
                diagnosticRow("suppressPresentation", appState.candidatePresentationMode == .suppressAnswers ? "true" : "false")
                diagnosticRow("suppressCandidateQuestions", appState.candidateAsksPanelMode == .suppressAnswers ? "true" : "false")
