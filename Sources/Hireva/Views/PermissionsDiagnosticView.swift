...
struct PermissionsDiagnosticView: View {
    ...
    private var migrationDiagnosticsCard: some View {
        ...
        return VStack(alignment: .leading, spacing: 10) {
            Label("Hireva Data Migration", systemImage: "externaldrive.badge.checkmark")
                .font(.headline)
    ...
    private var screenRecordingRestartBanner: some View {
        ...
        case .identityMismatch:
            VStack(alignment: .leading, spacing: 12) {
                ...
                Text("Verify you are running from a signed bundle inside 'dist/Hireva.app'.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            .padding(18)
    ...
    private var tccResetInstructionsCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("tccutil reset Microphone com.langcheng.Hireva")
                Text("tccutil reset SpeechRecognition com.langcheng.Hireva")
                Text("tccutil reset ScreenCapture com.langcheng.Hireva")
            }
            ...
            VStack(alignment: .leading, spacing: 8) {
                Group {
                    Text("# 1. Verify Info.plist Bundle Identifier")
                        .foregroundStyle(.secondary)
                    Text("defaults read \"$(pwd)/dist/Hireva.app/Contents/Info.plist\" CFBundleIdentifier")
                }
                
                Group {
                    Text("# 2. Verify Code Signature and Entitlements")
                        .foregroundStyle(.secondary)
                    Text("codesign -dvvvv dist/Hireva.app")
                }
                
                Group {
                    Text("# 3. Verify Designated Requirement")
                        .foregroundStyle(.secondary)
                    Text("codesign -d -r- dist/Hireva.app")
                }
                
                Group {
                    Text("# 4. Check Running Instances and Process Paths")
                        .foregroundStyle(.secondary)
                    Text("ps aux | grep -E \"Hireva|Contents/MacOS\" | grep -v grep")
                }
                
                Group {
                    Text("# 5. Reset All Audio & Screen TCC Permissions")
                        .foregroundStyle(.secondary)
                    Text("tccutil reset Microphone com.langcheng.Hireva && tccutil reset ScreenCapture com.langcheng.Hireva && tccutil reset SpeechRecognition com.langcheng.Hireva")
                }
            }
            .font(.system(size: 10, design: .monospaced))
            .padding(10)
