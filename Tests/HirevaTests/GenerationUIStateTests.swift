...
struct GenerationUIStateTests {
    ...
    @Test
    func delayedPersistenceForPreviousSessionDoesNotReplaceLiveHistory() async throws {
        let (appState, session, question, delayProvider) = try makeAppState(
        ...
        delayProvider.sleepDuration = 0
        appState.generationFullCardWatchdogNanoseconds = 2_000_000_000
        appState.simulatedSuggestionPersistenceDelayNanoseconds = 500_000_000
        ...
        let nextSession = try appState.createContextBoundSession(mode: .mock, title: "Next Context")
        appState.currentSession = nextSession
        appState.liveSuggestionHistory = []

        try await Task.sleep(nanoseconds: 800_000_000)

        #expect(try appState.suggestionRepository.suggestions(sessionID: session.id).contains {
            $0.questionID == question.id
        })
        #expect(appState.currentSession?.id == nextSession.id)
        #expect(appState.liveSuggestionHistory.isEmpty)
    }

    @Test
    func supersededGenerationFailureCannotPropagateToCurrentQuestion() throws {
        let (appState, session, firstQuestion, _) = try makeAppState(client: ConsecutiveQuestionLLMClient())
        let secondQuestion = try makeQuestion(
            sessionID: session.id,
            text: "Which reliability signal would you investigate first and why?",
            suffix: "failure-ownership"
        )
        ...
        #expect(!appState.shouldPropagateGenerationFailure(
            generationID: "generation-one",
